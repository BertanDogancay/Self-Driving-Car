...
class Lane:
  ...
  def __init__(self, orig_frame):
    ...
    self.roi_points = np.float32([
      (250,300), # Top-left corner
      (0, height), # Bottom-left corner            
      (width,height), # Bottom-right corner
      (550,300) # Top-right corner
    ])
  ...
  def get_line_markings(self, frame=None):
    ...
    """
    def canny_algorithm(self, frame):
    ...
    def region_of_interest(canny_image):
      ...
      polygons = np.array([[(000, height), (900, height), (350, 250)]])
      mask = np.zeros_like(canny_image)
      cv2.fillPoly(mask, polygons, 255)
      masked_image = cv2.bitwise_and(canny_image, mask)
      return masked_image

    canny_image = canny_algorithm(self, frame)
    masked_image = region_of_interest(canny_image)
    self.lane_line_markings = masked_image
...
def auto_control(center_offset, motor):
  motor.speed = 40
  if -10 < center_offset < 10:
    motor.move_forward()
...
def main():
    ...
    while (cap.isOpened()):
        ...
        motor = mt.motor_control()

        auto_control(center_offset, motor)
